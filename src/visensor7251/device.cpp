...
static mutex mutex_imu;
static mutex mutex_img;
static queue<imuData> imu_queue;
static condition_variable con_imu;
static condition_variable con_img;
...
bool device::readIMU(imuData *imudata){
  std::unique_lock<std::mutex> lk(mutex_imu);
  con_imu.wait(lk, [&]
	   {
